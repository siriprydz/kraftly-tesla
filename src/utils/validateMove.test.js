...
describe('validateMove', () => {
  ...
  it.each(['1234', '123456', 'ABCDE', '80-267'])('rejects zip code %s', (zip) => {
  ...
  it('validates the date format', () => {
    expect(validateMove({ ...validMove, date: '2026/10/01' }, today)).toMatchObject({
      date: 'Datum måste vara i formatet ÅÅÅÅ-MM-DD',
    })
  })

  it("rejects a date that doesn't exist on the calendar", () => {
    expect(validateMove({ ...validMove, date: '2026-02-30' }, today)).toMatchObject({
      date: 'Datum måste vara i formatet ÅÅÅÅ-MM-DD',
    })
  })
  ...
  it('returns errors for multiple invalid fields at once', () => {
    expect(validateMove({ ...validMove, address: '', zip: '123' }, today)).toEqual({
